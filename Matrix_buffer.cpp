#include <iostream>

class Buffer
{
    friend class Matrix;

private:
    int size = 0;
    double *t;
    int cpt = 1;

public:
    Buffer(int s, double v = 0.0) : size(s)
    {
        t = new double[size];
        for (int i = 0; i < size; i++)
        {
            t[i] = v;
        }
    }

    ~Buffer()
    {
        delete[] t;
    }

    void set(int h, double val)
    {
        t[h] = val;
    }

    double get(int h)
    {
        return t[h];
    }

    void decr_compteur()
    {
        cpt--;
        if (cpt == 0)
        {
            delete this;
        }
    }

    void incr_compteur()
    {
        cpt += 1;
    }
};

class Matrix
{
    int lignes = 0;
    int cols = 0;
    Buffer *pbuf;

    int flatten_index(int i, int j)
    {
        return (i * cols + j);
    }

public:
    Matrix(int i, int j, double val = 0) : lignes(i), cols(j)
    {
        if (i * j == 0)
        {
            pbuf = nullptr;
        }
        else
        {
            pbuf = new Buffer(i * j, val);
        }
    }

    Matrix(int i, int j, Buffer *b) : lignes(i), cols(j), pbuf(b)
    {
        b->cpt = b->cpt + 1;
    }

    Matrix(const Matrix& m1) : lignes(m1.lignes), cols(m1.cols), pbuf(m1.pbuf)
    {
        (m1.pbuf)->cpt += 1;
    }

    void set(int i, int j, double val)
    {
        pbuf->set(flatten_index(i, j), val);
    }

    double get(int i, int j)
    {
        return pbuf->get(flatten_index(i, j));
    }

    void print()
    {
        for (int i = 0; i < lignes; i++)
        {
            for (int j = 0; j < cols; j++)
            {
                std::cout << get(i, j) << " ";
            }
            std::cout << " " << std::endl;
        }
        std::cout << " " << std::endl;
    }

    Matrix reshape(int l, int c)
    {
        pbuf->cpt += 1;
        Matrix h(0, 0);
        h.cols = c;
        h.lignes = l;
        h.pbuf = pbuf;
        return h;
    }

    Matrix operator+(Matrix &B)
    {
        Matrix Sum(lignes, cols);
        for (int h = 0; h < lignes * cols; h++)
        {
            (Sum.pbuf->t)[h] = (pbuf->t)[h] + (B.pbuf->t)[h];
        }
        return Sum;
    }

    Matrix operator*(double l)
    {
        Matrix Prod(lignes, cols);
        for (int h = 0; h < lignes * cols; h++)
        {
            (Prod.pbuf->t)[h] = l * (pbuf->t)[h];
        }
        return Prod;
    }

Matrix& operator=(const Matrix& m)
{
    if (this != &m)
    {
        // Je sécurise le nouveau buffer
        m.pbuf->incr_compteur();

        // Ensuite on libére l'ancien
        pbuf->decr_compteur();

        // Et on mets les données
        lignes = m.lignes;
        cols   = m.cols;
        pbuf   = m.pbuf;
    }
    return *this;
}

    ~Matrix()
    {
        pbuf->cpt -= 1;
        if (pbuf->cpt == 1)
        {
            delete pbuf;
        }
    }
};

int main()
{
    Matrix m4(2, 4);
    Matrix m5(m4);

    m4.print();
    m5.print();

    return 0;
}
